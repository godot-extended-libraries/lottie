...
void VGGradient::set_color_ramp(const Ref<Gradient> &p_color_ramp) {
	if (color_ramp.is_valid()) {
		color_ramp->disconnect_compat(CoreStringNames::get_singleton()->changed, this, "_gradient_changed");
	}

	color_ramp = p_color_ramp;

	if (color_ramp.is_valid()) {
		color_ramp->connect_compat(CoreStringNames::get_singleton()->changed, this, "_gradient_changed");
	}
