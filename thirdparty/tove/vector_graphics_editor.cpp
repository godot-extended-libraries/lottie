...
Array VGTool::subpath_points_array(const tove::SubpathRef &subpath) {
	...
	virtual bool forward_gui_input(const Ref<InputEvent> &p_event) {
			...
			if (mb->get_button_index() == BUTTON_LEFT) {
				if (mb->is_pressed() && path == NULL) {
					...
					if (path && (global_to_local(root, mb->get_position()) - point0).length() < 2.0f) {
						...
						//undo_redo->add_do_method(root, "set_child_fill_color", n, VGTool::subpath_points_array(tove_subpath));
						//undo_redo->add_do_method(root, "set_child_line_color", n, VGTool::subpath_points_array(tove_subpath));
						//undo_redo->add_do_method(root, "set_child_points", n, VGTool::subpath_points_array(tove_subpath));
		...
		if (path && mm.is_valid()) {
			if (mm->get_button_mask() & BUTTON_MASK_LEFT) {
				...
				path->set_points(0, VGTool::subpath_points_array(tove_subpath));
...
Array VGCurveTool::_get_points(const SubpathId &p_id) {
	...
	return VGTool::subpath_points_array(path->getSubpath(p_id.subpath));
}
...
bool VGEditor::forward_gui_input(const Ref<InputEvent> &p_event) {
	...
	if (buttons[TOOL_TRANSFORM]->is_pressed()) {
		Ref<InputEventMouseButton> mb = p_event;
...
void VGCurveTool::remove_point(const Vertex &p_vertex) {
	...
	if (subpath->getNumPoints() >= 4) {
		...
		Array previous = VGTool::subpath_points_array(subpath);
